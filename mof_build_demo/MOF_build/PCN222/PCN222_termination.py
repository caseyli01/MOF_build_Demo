...
def filter_abc_terminations(cut_term,a,b,c,carte_points,tric_points,x_max,y_max,z_max,x_min,y_min,z_min):
    ...
    ab_filtered_min = filt_boundary_cut_res(cut_term, b, a)
    ac_filtered_min = filt_boundary_cut_res(cut_term, c, a)
    bc_filtered_min = filt_boundary_cut_res(cut_term, b, c)
    ab_filtered_max = filt_boundary_cut_res(cut_term, -b, a)
    ac_filtered_max = filt_boundary_cut_res(cut_term, -c, a)
    bc_filtered_max = filt_boundary_cut_res(cut_term, c, b)
...
def get_term(
    ...
    a,b,c = np.array([1, 0, 0]),        np.array([0, 1, 0]),        np.array([0, 1, 2])
